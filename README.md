...
| `colortest.py` | displays 256 colors with their codes in the terminal |
| `compiler` | compile a document to another format (mainly used for LaTeX) |
| `convert_library` | convert a list of movie files to modern HEVC x265 |
...
| `scan_library` | scan a whole library of video files and prepare a list to be converted to modern HEVC x265 |
...
* *Kitty* works like *Alacritty* but does a much better job at rendering unicode characters and ligatures: this is the main reason I keep it around. It has much more features that I don't use. It has almost the same configuration as *Alacritty*, no extra plugins/*kittens*
...
Doing this allows modularity: for example, a shortcut opens the Web browser defined by the *$BROWSER* variable. This variable sets *Firefox* as the primary browser, but then several fallbacks such as *Brave*, *Google Chrome*, etc... A default fallback option is also hard-coded in case the variable does not exit.
